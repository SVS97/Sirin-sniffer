/*
 * Sniffer for ​collects​ ​ statistic​ ​ about​ ​ network​ ​ traffic
 * version 1.0
 ...
 * A (very) simple internet sniffer written in C for training purposes. 
 * Project compiles with Makefile help (make all). 
 * More about pipes: https://goo.gl/JXoeJm, about Daemons: https://goo.gl/Rg4Ucp
 */ 
...
int main(int argc, char *argv[])
{
	...
	int argopt;
	while ((argopt = getopt(argc, argv, "ut")) != -1){			/* Choosing arguments 				*/
		switch(argopt) {
		case 'u':
	...
	logfile = fopen("log.txt","w");						/* Output file with sniff data */
	if(logfile == NULL)
	...
	if (pid == (pid_t) 0){
		read_from_pipe (mypipe[0]);					/* This is child process 	*/
 	}
	else if (pid < (pid_t) 0){
...
/* Counting and printing packets in file
 * @ *buffer - pointer to buffer with information
 * @ size - size of packet
 */
...
/* Printing TCP packet information
 * @ *Buffer - pointer to buffer with information
 * @ Size - size of packet
 */
...
/* Printing UDP packet information
 * @ *Buffer - pointer to buffer with information
 * @ Size - size of packet
 */
...
/* Printing ICMP packet information
 * @ *Buffer - pointer to buffer with information
 * @ Size - size of packet
 */
 ...
 * @ file - file with information (FIFO)
*/
